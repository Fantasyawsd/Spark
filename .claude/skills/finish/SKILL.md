...
5. **在 main 完成最终归档**
   ...
   - 按 `docs/standards/version-control.md`「验证门禁」完成合并后双目标构建：development APK 与 Windows release EXE 必须在同一次 `/finish` 流程中均构建成功；记录两个产物的路径、大小和 SHA-256，任一失败不得继续归档或清理。
   ...
   - 完成标准：任务提交和合并后文档归档提交均已进入 `main`；APK 与 Windows release EXE 构建证据完整；台账不再停留在“待合并”或“进行中”；开发计划反映真实代码状态。
