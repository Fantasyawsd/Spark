...
- 论文聊天主要使用论文元信息和摘要；完整 PDF 上下文依赖论文模块的 PDF 解析能力
...
| # | 任务 | 状态 | 说明 |
| --- | --- | --- | --- |
| 1 | 稳定流式 Markdown / 公式 / 代码 / 来源的增量渲染 | 🚧 持续打磨 | 基础已具备，持续改进边界场景 |
| 2 | 明确主聊天、论文聊天与 AI 解读缓存之间的数据边界 | ✅ 已完成 | 边界规则见 §4.2「ChatPaper 上下文边界」；测试覆盖主聊天/论文聊天身份与注入规则 |
| 3 | 接入 PDF 分块、可追溯引用和上下文裁剪 | ⬜ 待开始 | 依赖论文模块第 9 条 |
| 4 | 会话级系统提示词、个性化偏好与可组合 Skills | ⬜ 待规划 | 先定义作用域、优先级、持久化与安全边界，再接入聊天运行时 |
| 5 | 选择模型二级菜单头像化 | ⬜ 待规划 | 模型二级菜单与 Composer 模型入口统一展示模型头像、名称和 provider 信息；模型头像去除自身背景色、透明显示；加载失败保留 fallback |
| 6 | 网络搜索引用默认折叠 | ⬜ 待规划 | 来源/引用列表默认收起，用户主动展开后查看详情；保留来源数量、可点击链接和错误状态 |
| 7 | 发送后收起键盘，AI 回复期间不自动聚焦回输入区 | ⬜ 待规划 | 当前发送后，AI 回复完成时会重新聚焦输入区并弹出系统键盘（部分设备卡顿）；应发送即收起键盘，回复过程中保持收起，用户需要时再主动点按输入区 |
