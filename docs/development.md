...
4. ✅ 实时热点（Phase 3）与个性化推荐（Phase 4）已按 §3.1 完成：Trending 三信号 + Web Heat + Trend Boost、设备本地行为日志 → 匿名画像 → Personalized Pool 混排。

当前阶段：

- 保留论文 AI 解读与 ChatPaper PDF 线的既有设计，但本阶段暂缓实现；继续维护相关论文、side chat 和个人数据恢复等客户端任务。
- 高级推荐（Phase 5：多路召回、Two-Tower、Learning to Rank 等）为后续阶段。
...
| 我的 | 提供收藏与历史入口、主题外观、本地数据管理、DeepSeek Key 管理、隐私说明和开源许可；个性化推荐开关（关闭即停采）与「清除行为数据」入口 |
...
- Phase 2/2.5 已完成真实库端到端收口：服务端、development Client、会议频道、推荐净增量与已读过滤均通过自动化和 Windows App 人工验收；production Client 数据源保持 arXiv Atom 不变。
...
- ChatPaper 的未完成回复可在应用进程存活且平台未挂起网络任务时跨界面继续；强制退出、系统杀进程或系统挂起后的续跑仍需未来服务端异步任务与恢复协议。
- 社区和私信仅保留未接入生产导航的实验代码；Hugging Face Blogs 后续作为社区文章来源，不进入论文频道。
- 个性化隐私边界（Phase 4）：行为事件仅保存在设备本地（90 天保留期、10000 条上限），推荐请求只上送聚合后的匿名偏好画像、不上送原始行为；用户可关闭采集（停止记录）并清除行为事件与画像；behavior 数据纳入本地数据统计与清理。
- LLM Trend Scout 已实现发现-核验-写入闭环（含 HTTP 适配器与测试），但尚未接入 CLI/定时调度，真实库暂无 web_heat 信号；Trending 当前由 HF 热度、star 增速与引用增速驱动。
...
| 1.6 | 索引与只读 Paper API | 已完成真实全量索引、查询性能和 API 验证；Windows App 端到端人工验收已由 Phase 2.5 收尾 |
...
- 人工收口：Windows development App 已由编排者确认真实论文、推荐刷新后旧列表保留且新 batch 向下净新增、已读过滤，以及会议频道可添加并返回内容。
...
| # | 任务 | 状态 | 说明 |
| --- | --- | --- | --- |
| 1 | 离开聊天界面后继续完成回复 | 已完成 | 主聊天、论文全屏聊天和论文内嵌讨论按会话共享应用级生成任务；重入、失败重试、删除与清理竞态已有测试和 Windows 人工验收 |
| 2 | 修复论文聊天“读取全文”异常反馈 | 暂缓 | 随 PDF 线暂缓；恢复时要求所有下载/解析失败路径复位 loading，并覆盖无 PDF、超时、HTTP 错误和解析异常测试 |
| 3 | 主聊天侧边追问（side chat） | 已完成 | 从主聊天当前状态 fork 临时会话，解决主线探索中的概念追问；不污染主聊天，退出即删除，交互与边界见 §4.3；欢迎标题“临时会话”已按验收统一 |
| 4 | 临时会话保存为持久会话 | 待开始 | side chat 内提供保存按钮，将当前临时会话转为持久化会话并进入会话列表；需明确去重、命名与存储契约 |
| 5 | 会话引用（跨会话引用） | 待开始 | 支持在一个会话中引用其他会话的内容/上下文；需定义引用粒度、展示与存储形式，避免上下文污染与循环引用 |
...
| 4 | 书单功能 | 待开始 | 支持创建、编辑、删除书单，向书单加入/移除论文并查看书单内论文；排序、批量操作与跨设备同步另行定义 |
...
- AI 与用户消息使用 GFM + LaTeX；代码块支持 Atom One Light / Atom One Dark 主题、语言高亮、JetBrains Mono、本地复制和容器内自动换行
- 未知语言或高亮失败时完整回退为可读纯文本；代码、公式与 Markdown 失败时仍有可读降级
- 每个会话的上下文、思考开关和联网状态行为明确且可恢复
- 应用进程存活时，离开并返回同一持久会话不会取消或重复未完成的 AI 回复；删除会话和清理聊天数据不会被迟到回复复活
