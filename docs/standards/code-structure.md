...
> 状态：强制执行  
> 适用范围：Spark 全部生产代码、测试代码和后续重构  
> 最近更新：2026-08-03

## 1. 目的

本文规定 Spark 的代码组织、模块边界和依赖方向。后续功能开发与代码审查必须遵循这些规则，以继续开发和长期可维护性为主要目标。
...
charter 的判断测试：把 `core/` 搬到另一个应用，再配上新业务代码，就构成另一个应用——`core/` 不应携带任何 Spark 业务语义。
...
- `core/` 子目录的公开文件（如 `SparkColors`、`SparkTheme`、通用组件）就是它的公共入口，业务模块不访问其私有实现文件。
