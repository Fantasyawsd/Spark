...
PaperFlow 已建立代码、发布、数据、接口和功能五层版本管理的统一规则与当前阶段可执行的基础设施。Android 现在区分 development、staging、production 三个渠道；应用版本由 `pubspec.yaml` 管理，工具同步应用内展示并由 CI 校验；GitHub Actions 对 Pull Request 和 `main` 执行版本、格式、分析、测试和 development APK 构建门禁。
...
- Android：新增三个 product flavor，以不同 applicationId、versionName 后缀和应用标签隔离安装渠道。
- 版本工具：新增递增版本、校验版本和检查变更 Dart 文件格式的 PowerShell 脚本。
- CI：新增 GitHub Actions，执行依赖、版本、格式、分析、测试、development APK 构建和 artifact 上传。
...
| `.\tool\verify_changed_dart_format.ps1` | 9 个变更 Dart 文件通过 |
| PowerShell Parser | 三个工具脚本无语法错误 |
| Markdown 本地链接检查 | 通过 |
| `flutter analyze` | 通过，无问题 |
| `flutter test` | 194 项全部通过 |
| development debug APK | 成功；`app.paperflow.reader.dev` / `0.1.0-dev` / PaperFlow Dev |
| staging debug APK | 成功；`app.paperflow.reader.staging` / `0.1.0-beta` / PaperFlow Beta |
...
- 构建兼容性：后续 Android 命令必须明确 flavor 与匹配的 `PAPERFLOW_ENV`。

## 已知风险与回滚

- GitHub Actions 尚未在远程 PR 上实际运行；本地已覆盖其核心命令，远程 runner 和 artifact 上传仍需首次 PR 验证。
- flavor 与 `PAPERFLOW_ENV` 由构建命令配对，Gradle 目前不能在运行时识别错误的手工组合；只使用文档和 CI 中的规范命令。
...
- 首次推送本分支后观察 GitHub Actions，并确认 APK artifact 上传路径。
