...
- 合并方式：本地快进合并（`main` `ba6c879..e8ea4ff`）
- 最终集成提交：`e8ea4ff`
- 合并时间：2026-08-14
- 集成验证：合并后 `git diff --check` 与 Markdown 链接检查通过；纯文档任务按 AGENTS.md §10 不执行 Flutter 双目标构建。
- 开发计划影响：本任务仅对齐文档与代码事实，不改变产品能力，`docs/development.md` 无需更新。
- 真实后续项：无；归档后本台账除勘误外不再更新。
