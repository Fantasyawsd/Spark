...
- 开发计划影响：`docs/development.md` 无需更新——本任务未引入新的数据契约或产品边界；滑到底刷新复用既有推荐刷新语义（Phase 2 已记载「刷新排除当前列表与已读、新 batch 向下净新增」），单栏切双栏定位为纯客户端交互细节，chips 整体回退后文档描述的徽标行为（无信号零展示变化）保持不变。
