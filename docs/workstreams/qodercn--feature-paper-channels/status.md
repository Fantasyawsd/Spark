...
- 审查日期：2026-08-04
- 阻断项：无
- 缺陷：验收期间反馈均已修复（切换刷新、频道缓存、双栏进入方式、管理页形态）
- 结论：可合并（编排者经 Windows 桌面多轮实操验收后批准直接进入合并，豁免独立 `/review` 环节）
...
论文页具备结构化频道浏览能力，与计划一致并包含验收期间的编排者调整：

- `Paper` 元数据语义修正：`source`/`venue`/`journalReference`/`comment`/`affiliations` 分离，arXiv Subjects 结构化（primary + 全部），引用数未知时不再显示「被引 0」；
- 频道体系：固定频道（推荐/关注/最新）+ 用户主题频道（首批 4 个结构化 arXiv 主题），频道偏好版本化持久化（`papers.channel-preferences` v1）；
- 频道栏：文字 + 选中下划线、横向滚动、「＋」固定右侧；「＋」打开频道管理页（主题/会议 Tab，可左右滑动，对勾即添加状态，会议页未开放）；
- 每个频道保存各自查询加载的论文列表、浏览位置与分页状态；切换频道仅懒加载未加载频道，强制刷新只由下拉触发；
- 双栏点击论文改为临时进入详情页，可返回双栏。

验收调整（均经编排者确认）：切换不刷新、主题搜索暂缓、管理页 Tab 化、文案「主题/会议」、对勾代替已添加区块、双栏临时进入、频道独立缓存。

### 实际变更

- 领域与业务逻辑：`Paper` 实体字段拆分与未知语义（含 `RelatedPaper.venue` 可空）；`UserPaperChannel`/`FixedPaperChannel`/`PaperChannelPreferences` 契约；`ArxivSubjectCatalog` 首批主题；`PaperFeedController` 频道模型（懒加载、每频道独立列表、位置与选中态持久化、旧偏好回退）；`PaperFeedFilter` 精简为关注/最新语义。
- 数据与基础设施：arXiv Atom/OAI 映射与种子语义更新；论文缓存 `papers.catalog-cache` schema v2 与 1→2 迁移；频道偏好文件仓储 + 内存实现；组合根接线并纳入本地数据清理清单。
- 界面与交互：`papers_header.dart` 重写为频道栏；新增 `paper_channel_manager_sheet.dart`；`papers_screen.dart` 接线与双栏临时详情；删除 `paper_category_picker.dart`；展示层引用数未知时隐藏。
- 测试与工具：测试数 204 → 218，覆盖映射语义、缓存迁移、频道偏好、管理页交互、频道列表独立与切换不重复请求。
- 文档：台账全程记录决策、验证与检查点。

### 兼容性与迁移

- 本地数据迁移：论文缓存自动执行 `papers.catalog-cache` 1→2 迁移（还原 venue/affiliation/citations 未知语义）；频道偏好为全新文件；旧论文偏好的 `extraTopics`/`topicIndex` 字段保留读取兼容但不再使用，`primaryCategoryIndex` 作为频道选中回退。
- API 或领域契约变化：仅应用内领域契约（`Paper` 字段拆分），无对外 API。
- 旧版本兼容性：应用未正式发布（0.0.1+1），无旧版本兼容负担；旧缓存可被新代码读取并自动升级。

### 已知风险与回滚

- 已知风险：旧自由文本主题不迁移（既定决策）；离线种子主题交叉较多，离线时不同主题频道内容可能重叠（数据源特性）；会议频道暂无真实来源，仅展示说明。
- 回滚方式：整体 revert 分支提交 `1b3f8bb..HEAD` 即可恢复旧行为；注意回滚后旧代码遇到 schema v2 论文缓存会拒绝读取并隔离该文件，目录缓存将重建，互动/评论等其他本地数据不受影响。

### 文档更新建议

- 更新 `docs/development.md` 论文领域：§2.1 当前能力补充频道体系；§2.4 标注开发顺序 1–3 已完成，频道管理页「搜索与排序」待主题目录扩大后恢复。

### 未完成与后续工作

- 时间筛选入口改造（开发顺序步骤 4）。
- 真实 arXiv 主题查询接入与缓存键/分页/位置恢复验证（步骤 5）。
- 会议数据源端口与首个真实会议提供方纵向闭环（步骤 6），届时开放会议 Tab。
- 主题目录扩大后恢复频道搜索与排序入口；清理旧偏好 `extraTopics`/`topicIndex` 字段。

## 合并归档

- 最终状态：已合并
- 合入分支：`main`
- 最终集成提交：`a9d11ad`（历史含上游 main 合并，任务提交已可达 main）
- Pull Request：无
- 合并时间：2026-08-04 01:31（+08:00）
- main 集成验证：`git merge-base --is-ancestor af98080 main` 通过；原台账记录 218 项测试、频道交互验收和 APK 构建门禁通过
- 开发计划更新：已核对 `docs/development.md` §2.1、§3.1 第 1–5 项，频道模型、频道栏、时间筛选和元数据语义均已记录
- 最终后续项：真实会议数据源、会议 Tab 开放、目录扩大后的搜索排序和旧偏好字段清理
