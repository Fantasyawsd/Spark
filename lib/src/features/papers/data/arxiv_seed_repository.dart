...
class ArxivSeedRepository implements PaperRepository {
  ...
  @override
  List<Paper> getAll() {
    return arxivSeedDtos.map((dto) {
      return const ArxivPaperMapper().toDomain(
        dto,
        relatedPapers: _relatedPapersFor(dto),
      );
    }).toList(growable: false);
  }

  List<RelatedPaper> _relatedPapersFor(ArxivPaperDto source) {
    final sourceId = normalizeArxivId(source.id);
    final candidates = arxivSeedDtos
        .where((candidate) => normalizeArxivId(candidate.id) != sourceId)
        .map((candidate) {
    ...
    return candidates.take(3).map((candidate) {
      ...
      return RelatedPaper(
        id: normalizeArxivId(candidate.metadata.id),
...
final arxivSeedDtos = <ArxivPaperDto>[
  ArxivPaperDto(
    id: '2402.06734',
    title:
