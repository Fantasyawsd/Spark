...
void main() {
  ...
  test('restores cancelled assistant messages after repository recreation',
      () async {
    final first = FileChatSessionRepository(
    ...
    final restored = FileChatSessionRepository(
      store: LocalJsonStore(fileName: 'unused.json', file: file),
    );
    ...
    expect(messages.last.status, ChatMessageStatus.cancelled);
  });
...
''');
    final repository = FileChatSessionRepository(
      store: LocalJsonStore(fileName: 'unused.json', file: file),
    );

    final messages = await repository.load('paper-1');

    expect(messages.single.status, ChatMessageStatus.complete);
  });
