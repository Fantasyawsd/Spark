...
void main() {
  test('reads nullable primitive collections with stable defaults', () {
    expect(PaperJsonValueReader.stringList(const {}, 'items'), isEmpty);
    ...
    expect(PaperJsonValueReader.nullableInt(const {}, 'count'), isNull);
    expect(PaperJsonValueReader.optionalBool(const {}, 'enabled'), isFalse);
    expect(PaperJsonValueReader.nullableString(const {}, 'label'), isNull);
  });
  ...
  test('rejects values with an incompatible primitive shape', () {
    ...
    expect(
      () => PaperJsonValueReader.requiredStringList(const {}, 'items'),
      throwsA(isA<FormatException>()),
    );
    expect(
    ...
    expect(
      () => PaperJsonValueReader.optionalBool({'enabled': 'yes'}, 'enabled'),
      throwsA(isA<FormatException>()),
    );
  });
}
